...
TOKEN='5242382351:AAHHg_w3b6kjkmTizd7hb_EtwMrcIm2lj8Y'


def start(update: Update, context: CallbackContext) :
    welcome='Hi bitch in order to use this bot only click on the button which is marked "CLICK" lol u dumb bitch Nastia '
...
def homo_sexual():
    ...
    text= " Я на <b>%s</b>"%percent+"<b>%</b>"+" гамасєк (LGBTQIA+💅) 🏳‍🌈"
    return text
...
def inlinequery(update: Update, _: CallbackContext) :
    results = [
        ...
        InlineQueryResultArticle(
            id=str(uuid4()),
            title="тіпи веджайн ШОК КОНТЕНТ",
...
def info(update: Update, context: CallbackContext) :
    update.message.reply_text('This is really simple bot which is written in python by | @kyrylo0 | \n If you want source code  text me')
def main() :
